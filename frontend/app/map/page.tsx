...
function MapPage() {
  ...
  const [showControls, setShowControls] = useState(false)  // mobile controls drawer
    ...
    <div className="space-y-4">
      <div>
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input className="input pl-8 text-sm" placeholder="KIT Campus, Berlin…" value={centerInput}
      ...
      <div>
        ...
        <input type="range" min={1} max={30} step={1} value={radiusKm}
          onChange={e => setRadiusKm(Number(e.target.value))} className="w-full accent-indigo-600" />
        <div className="flex justify-between text-xs mt-1" style={{ color: "var(--text-3)" }}>
          <span>1 km</span><span>30 km</span>
        </div>
      </div>

      <button onClick={() => setShowFilters(v => !v)} className="w-full flex items-center justify-between py-1">
        <span className="section-label flex items-center gap-1.5">
          <SlidersHorizontal size={12} /> Filters
          {activeFilterCount > 0 && <span className="bg-indigo-600 text-white text-[10px] px-1.5 py-0.5 rounded-full font-bold">{activeFilterCount}</span>}
        </span>
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {[["", "Any"], ["möbliert", "Furnished"], ["teilmöbliert", "Part-furn."], ["unmöbliert", "Unfurnished"]].map(([val, label]) => (
                <button key={val} onClick={() => setSetup(setup === val ? "" : val)}
                  ...
                  style={{ background: setup === val ? "var(--accent)" : "var(--surface-2)", color: setup === val ? "white" : "var(--text-2)", border: `1px solid ${setup === val ? "var(--accent)" : "var(--border)"}` }}>
                  {label}
                </button>
              ))}
            </div>
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-4" style={{ height: "calc(100vh - 7rem)" }}>

      {/* ── Desktop sidebar (hidden on mobile) ── */}
      <div className="hidden sm:flex w-72 shrink-0 flex-col gap-3 overflow-y-auto">
      ...
      <div className="flex-1 relative min-h-0">
        ...
        <div className="sm:hidden absolute top-3 left-3 right-3 z-[500]">
          <div className="bg-white rounded-xl shadow-lg flex gap-2 p-2" style={{ border: "1px solid var(--border)" }}>
            <div className="relative flex-1">
              <Search size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2" style={{ color: "var(--text-3)" }} />
              <input className="w-full text-sm outline-none pl-8 pr-2 py-1.5 bg-transparent" style={{ color: "var(--text-1)" }}
                placeholder="Search location…" value={centerInput}
                onChange={e => { setCenterInput(e.target.value); setGeoError("") }}
                onKeyDown={e => e.key === "Enter" && handleGeocode()} />
            </div>
            <button onClick={handleGeocode} disabled={geocoding} className="btn-primary px-2.5 py-1 shrink-0">
              {geocoding ? <Loader2 size={13} className="animate-spin" /> : <ChevronRight size={13} />}
            </button>
            <button onClick={() => setShowControls(true)} className="btn-secondary px-2.5 py-1 shrink-0 relative">
          ...
          {geoError && <p className="text-xs text-red-500 mt-1 bg-white rounded-lg px-3 py-1 shadow-sm">{geoError}</p>}
        </div>

        {/* ── Mobile: floating mode buttons ── */}
        <div className="sm:hidden absolute top-20 right-3 z-[500] flex flex-col gap-2">
          <button onClick={() => setMode(m => m === "setcenter" ? null : "setcenter")}
        ...
        {/* ── Mobile: bottom sheet ── */}
        <div
          className="sm:hidden absolute bottom-0 left-0 right-0 z-[500] bg-white rounded-t-2xl"
          style={{ border: "1px solid var(--border)", boxShadow: "0 -4px 20px rgba(0,0,0,0.08)" }}
        >
          <div className="flex justify-center pt-2.5 pb-1 cursor-pointer" onClick={() => setSheetExpanded(v => !v)}>
            <div className="w-9 h-1 rounded-full bg-gray-200" />
          </div>
          <div className="flex items-center justify-between px-4 py-2 cursor-pointer" onClick={() => setSheetExpanded(v => !v)}>
            <div className="flex items-center gap-4">
              <div><span className="font-bold text-base" style={{ color: "var(--text-1)" }}>{inRadius.length}</span><span className="text-xs ml-1" style={{ color: "var(--text-3)" }}>rooms</span></div>
              <div><span className="font-bold text-base" style={{ color: "var(--text-1)" }}>{inRadius.filter(p => p.has_email).length}</span><span className="text-xs ml-1" style={{ color: "var(--text-3)" }}>email</span></div>
              {inRadius.length > 0 && <div><span className="font-bold text-base" style={{ color: "var(--text-1)" }}>€{Math.min(...inRadius.map(p => p.rent))}</span><span className="text-xs ml-1" style={{ color: "var(--text-3)" }}>min</span></div>}
            </div>
            <div className="flex items-center gap-2">
              {inRadius.filter(p => p.has_email).length > 0 && (
          ...
          <div className="px-4 pb-3">
            <div className="flex items-center gap-3">
              ...
              <input type="range" min={1} max={30} step={1} value={radiusKm}
                onChange={e => setRadiusKm(Number(e.target.value))} className="flex-1 accent-indigo-600" />
              <span className="text-xs font-semibold shrink-0" style={{ color: "var(--accent)" }}>{radiusKm} km</span>
            </div>
          </div>

          {sheetExpanded && (
            <div className="overflow-y-auto" style={{ maxHeight: "40vh", borderTop: "1px solid var(--border)" }}>
              {inRadius.length === 0
                ? <p className="text-center py-6 text-sm" style={{ color: "var(--text-3)" }}>No rooms in this radius</p>
                : inRadius.slice(0, 20).map(p => (
                  <RoomRow key={p.id} pin={p} distance={p.dist}
        ...
        {/* Selected room card (desktop) */}
        {selectedRoom && (
          <div className="hidden sm:block absolute top-4 right-4 z-[500] w-64 card p-4 space-y-2.5" style={{ boxShadow: "var(--shadow-lg)" }}>
      ...
      {showControls && (
        <div className="sm:hidden fixed inset-0 z-[600] flex items-end" style={{ background: "rgba(0,0,0,0.5)" }}
          onClick={() => setShowControls(false)}>
          <div className="w-full bg-white rounded-t-2xl p-5 space-y-4" style={{ maxHeight: "80vh", overflowY: "auto" }}
            onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h3 className="font-semibold" style={{ color: "var(--text-1)" }}>Map Controls</h3>
              <button onClick={() => setShowControls(false)} className="btn-ghost p-1"><X size={17} /></button>
            </div>
            {filterPanel}
          </div>
        </div>
      ...
      {bulkRooms && (
        <BulkEmailModal rooms={bulkRooms} onClose={() => setBulkRooms(null)}
